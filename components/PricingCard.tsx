  ...
  return (
    ...
    >
      ...
      {plan.badge && !plan.popular && (
        <div className="absolute -top-3 left-1/2 -translate-x-1/2">
          <span className="px-4 py-1 bg-gradient-to-r from-yellow-500 to-yellow-600 text-slate-900 text-xs font-semibold rounded-full">
            {plan.badge}
          </span>
        </div>
      )}
      ...
      {/* Plan Name */}
      <h3 className={`text-xl font-bold mb-2 mt-2 ${isEarlyBird ? 'text-yellow-400' : 'text-white'}`}>{plan.name}</h3>
      ...
      {/* Price */}
      <div className="mb-6">
        <span className={`text-4xl font-bold ${isEarlyBird ? 'text-yellow-400' : 'text-white'}`}>£{plan.price}</span>
      ...
      <ul className="space-y-3 mb-8">
        {plan.features.map((feature, index) => (
          <li key={index} className="flex items-start text-sm">
            <svg
              className={`w-5 h-5 mr-2 flex-shrink-0 mt-0.5 ${isEarlyBird ? 'text-yellow-400' : 'text-teal-400'}`}
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
